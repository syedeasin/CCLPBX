...
Extendable with plugin support. Web interface authentication by default authenticates against the cclpbx Database. LDAP is one and has also been tested with Microsoft Active Directory an OpenLDAP.
...
Routes used to receive or send calls in or out of cclpbx.
...
cclpbx has unprecedented customizability which can be used to meet your needs or the needs of your customers. Customizable themes, menu, dialplan, and Hundreds of Default Settings to control the theme.
...
Can be configured for multi-master database replication, file replication. cclpbx, Database, and FreeSWITCH can be distributed across multiple servers for large enterprise scale systems.
...
.. _Contact Manager: http://docs.cclpbx.com/en/latest
